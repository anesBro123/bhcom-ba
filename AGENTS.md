...
- **Table filters (user + admin):** `shared/table/` flyout panel; **split-card chrome** (controls card: search + Filters toggle + chips band — desktop: chips + top-right **Clear filters**; mobile: Clear top-right then full-width compact chips, icon-only filter toggle with active dot); data card: table/pagination — default `tableChromeVariant='splitCard'`); per-filter accordion (`table-filter-field/`); `buildFilterSummary()` / `buildFilterChips()` with mobile `compact` truncation; **multi-select search autofocus**; **custom dual-thumb number-range** slider; **date-range** via `DatePeriodPickerComponent` (`titleKey: shared.table.filters.period`); defs in `portal/user/features/*/data/*-table-filters.ts` — see `table-filters.mdc`, `theme-tokens.mdc`
