...
- **Table filters (user + admin):** `shared/table/` flyout panel; **split-card chrome** (controls card for search/filters/chips + data card for table/pagination — default `tableChromeVariant='splitCard'`); per-filter accordion (`table-filter-field/`); `buildFilterSummary()` / `buildFilterChips()`; global layout SCSS `_table-chrome-split-card.scss`; defs in `portal/user/features/*/data/*-table-filters.ts` — see `table-filters.mdc`, `theme-tokens.mdc`
