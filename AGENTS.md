...
- **User find hub:** `portal/user/features/find/` — `EntityTabsComponent` + embedded marketplace tables (`/find?tab=`)
- **User our listings hub:** `portal/user/features/our-listings/` — tab-specific create CTA + embedded our tables (`/our-listings?tab=`)
...
- **Entity tabs (user hubs):** `shared/ui/entity-tabs/` — `EntityTabsComponent` (`app-entity-tabs`); config `portal/user/user-entity-tabs.config.ts`; programmatic nav: `userFindUrl()` / `userOurListingsUrl()`; template links: `userFindRoute()` / `userOurListingsRoute()` — see `entity-tabs.mdc`
