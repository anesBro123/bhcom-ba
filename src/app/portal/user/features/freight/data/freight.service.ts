...
import { FREIGHT_TABLE_FILTERS } from './freight-table-filters';
import { FREIGHT_MOCK_DATA } from './freight.mock-data';
import type { Freight, FreightFormModel } from './freight.model';

@Injectable({ providedIn: 'root' })
export class UserFreightService {
  private readonly authService = inject(AuthService);
  private readonly store: Freight[] = structuredClone(FREIGHT_MOCK_DATA);

  listOurs(query: TableQuery): Observable<PaginatedResponse<Freight>> {
  ...
  listAll(query: TableQuery): Observable<PaginatedResponse<Freight>> {
    const companyId = this.authService.user()!.companyId;
    const scoped = this.store.filter((item) => isExternalToCompany(item, companyId));
    return of(this.paginate(this.applyQuery([...scoped], query), query)).pipe(delay(200));
  }

  getById(id: string): Observable<Freight> {
    const item = this.store.find((entry) => entry.id === id);
    if (!item) {
      return throwError(() => new Error(`Freight not found: ${id}`)).pipe(delay(150));
    }

    return of(structuredClone(item)).pipe(delay(150));
  }

  create(payload: FreightFormModel): Observable<Freight> {
  ...
  update(id: string, payload: FreightFormModel): Observable<Freight> {
  ...
  private applyQuery(items: Freight[], query: TableQuery): Freight[] {
    let result = applyTableFilters(items, query, FREIGHT_TABLE_FILTERS);
  ...
  private paginate(items: Freight[], query: TableQuery): PaginatedResponse<Freight> {
