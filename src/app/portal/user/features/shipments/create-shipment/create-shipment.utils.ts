...
export function shipmentTypeLabelKey(type: string | undefined): string {
  switch (type) {
    case 'document_envelope':
      return 'portal.user.features.shipments.form.shipmentTypes.documentEnvelope';
    case 'freight_pallet':
      return 'portal.user.features.shipments.form.shipmentTypes.freightPallet';
    case 'bulk_cargo':
      return 'portal.user.features.shipments.form.shipmentTypes.bulkCargo';
    default:
      return 'portal.user.features.shipments.form.shipmentTypes.standardPackage';
  }
}
...
export function priorityLabelKey(priority: string | undefined): string {
  switch (priority) {
    case 'express':
      return 'portal.user.features.shipments.form.priority.express.title';
    case 'overnight':
      return 'portal.user.features.shipments.form.priority.overnight.title';
    default:
      return 'portal.user.features.shipments.form.priority.standard.title';
  }
}
...
export function selectedServicesLabels(value: Partial<CreateShipmentFormModel>): string[] {
  const labels: string[] = [];
  if (value.insuranceCoverage) labels.push('portal.user.features.shipments.form.services.insuranceCoverage');
  if (value.signatureRequired) labels.push('portal.user.features.shipments.form.services.signatureRequired');
  if (value.fragileHandling) labels.push('portal.user.features.shipments.form.services.fragileHandling');
  if (value.temperatureControl) labels.push('portal.user.features.shipments.form.services.temperatureControl');
  return labels;
}
